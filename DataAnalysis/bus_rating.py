...
def find_n_closest_stops(apt_loc, bus_stop_data,  n=5): # apt_loc=(lat,lon); bus_stop_data: list of (lat,lon)
    dist_set = []
    for i in range(len(bus_stop_data)):
        distance = dist_lon_lat(bus_stop_data.iat[i,2], bus_stop_data.iat[i,3], apt_loc[0], apt_loc[1])
        dist_set.append(distance)
    dist_set_order = sorted(dist_set)
    return dist_set_order[0:n]

    
def get_bus_rating(apt_loc, bus_stop_data):
    n_closest = find_n_closest_stops(apt_loc, bus_stop_data, n=5)
    rating = 0
...
def normalize_ratings(raw_ratings):
    ...
    with open("data/BusStopInfo.csv") as bus_info:
        bus_stop_data = pd.read_csv(bus_info)
        ...
        for i in range(len(apt_data)):
            ...
            raw_rating.append((apt_data.iat[i,0], get_bus_rating(apt_loc, bus_stop_data)))
        pkl.dump(raw_rating, open('/Users/Ceci/Desktop/raw_subway_scores.pkl', 'wb'))

with open("data/BusStopInfo.csv") as bus_info:
    bus_stop_data = pd.read_csv(bus_info)
    apt_data = pd.read_csv("data/aptZillow.csv")
    raw_rating = []
    for i in range(len(apt_data)):
        apt_loc = (apt_data.iat[i,1], apt_data.iat[i,2])
        raw_rating.append((apt_data.iat[i,0], get_bus_rating(apt_loc, bus_stop_data)))
    with open("raw_bus_scores2.csv", "w") as file:
        writer = csv.writer(file)
        for score in raw_rating:
            writer.writerow(score)
#raw_rating = pkl.load(open('/Users/Ceci/Desktop/raw_bus_scores.pkl', 'rb'))
#print("Raw Ratings")
#print(raw_rating)

norm_ratings = normalize_ratings(raw_rating)

with open("norm_bus_scores2.csv", "w") as file1:
...
#print(norm_ratings)
#pkl.dump(norm_ratings, open('/Users/Ceci/Desktop/normalized_bus_ratings.pkl', 'wb'))